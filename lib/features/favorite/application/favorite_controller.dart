...
final favoriteControllerProvider =
    AsyncNotifierProvider<FavoriteController, List<Movie>>(
      FavoriteController.new,
    );

class FavoriteController extends AsyncNotifier<List<Movie>> {
  @override
  Future<List<Movie>> build() async {
    final session = await ref.watch(authControllerProvider.future);
    if (session == null) return const [];

    return ref.watch(movieRepositoryProvider).fetchFavoriteMovies();
  }

  Future<FavoriteAction> toggle(Movie movie) async {
    final session = ref.read(authControllerProvider).asData?.value;
    if (session == null) {
      throw const ApiException('Sign in to use your favorites.');
    }
    ...
    try {
      ...
      state = AsyncData(
        _setMovieSaved(
          currentMovies,
          movie: movie,
          isSaved: action == FavoriteAction.added,
        ),
      );
